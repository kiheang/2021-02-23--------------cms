...
function settingdatepickerko(){
	...
	$.datepicker.regional['zh-TW'] = {
	  ...
	  monthNamesShort: ['一月','二月','三月','四月','五月','六月','七月','八月','九月','十月','十一月','十二月'],
...
function clipBoardCopy(){
	...
	if(browser_check().name == 'msie'){
		...
		alert('中_URL 주소가 복사되었습니다. 원하는 곳에 Ctrl+V로 붙여 넣기 하세요.');
	} else {
		prompt("中_이 글의 URL 주소입니다. Ctrl+C를 눌러 클립보드로 복사하세요", url);
	}
}
...
function getComboStr(resourceCodeList, valueColName, nameColName, selectedDtlCd, addOptionType){
	...
	if (addOptionType == 'C') {	//Choice
		rstlStr = '<option value="">请选择</option>';
	}
	else if (addOptionType == 'A') {	//All
		rstlStr = '<option value="">全部</option>';
	}
...
function fnComboStrFile(obj, num, max_count){
	...
	// 첨부파일 최대개수일때
	if(mc == num){
		html += '<span class="no_file_text">不能再登记</span>';
	}
	else {
...
function fnComboStrFileFront(obj, num, max_count){
	...
	// 첨부파일 최대개수일때
	if(mc == num){
		html += '<li><span class="no_file_text">不能再登记</span></li>';
	}
	else {
		...
		html += '</select>最多  ' + mc + '个, 到' + MAX_SIZE_STR + '</li>';
...
function ajaxJsonErrorAlert(jqXHR, textStatus, thrownError){
	if(jqXHR.status	==0){ 						alert("检验联网");
	}else if(jqXHR.status	==404){ 			alert("找不到窗口");
	}else if(jqXHR.status	==500){ 			alert("发生服务器有误");
	}else if(textStatus	=='parsererror'){ 	alert("应答报文分析失败");
	}else if(textStatus	=='timeout'){ 		alert("时间超过");
	}else {
		alert('出误');
	}
}
