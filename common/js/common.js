 ...
 * @type   : function
 * @access : public
 * @desc   : input 필드나 textarea 의 사이즈를 제한하여 지정된 길이(바이트수) 이상
 ...
 * @type   : function
 * @access : public
 * @desc   : input 필드나 textarea 의 사이즈를 제한하여 지정된 길이(글자수) 이상
 *           입력할 경우 추가입력된 메세지를 출력하고 추가 입력된 문자는 삭제함
 *
 * @param  : targetObj - Textarea Object
 * @param  : maxLength - Max Length(한글기준)
 * @return :
 */
function cfLengthCheck2(msg, targetObj, maxLength) {
    var len = 0;
    var newtext = "";
    for(var i=0 ; i < targetObj.value.length; i++) {
        var c = escape(targetObj.value.charAt(i));
...
function fnUrlCopyShare(url){
	...
	if (IE || IE11 !=-1)  {
		window.clipboardData.setData("Text", url);
		alert ( "주소가 복사되었습니다. \'Ctrl+V\'를 눌러 붙여넣기 해 주세요." );
		return;
	}
	else{
		temp = prompt(getNotiMsg('C_SV03'), url);
	}
}

//핫이슈, 정책참고, 정책반영 메뉴에 노출 여부 (검색시에는 노출됨)
function fnChangeExpsrAt(voteNo, showYn){
	var confirmMsg = "해당 투표를 메인에 노출시키시 겠습니까?";
	if(showYn == 'N'){
		confirmMsg = "해당 투표를 메인에 노출시키시 않겠습니까?";
	}
	
	if(confirm(confirmMsg)){
		var param = {'vote_no' : voteNo , 'expsr_at' : showYn};
		$.ajax({
			url : '/admin/vote/changeExpsrAtAjax.do',
			type : 'post',
			dataType : 'json',
			data : param,
			async : false,
...
function htmlTagFilterRestore(src) {
...
//자바스크립트 데이트 변환
//사용법 new Date().format("yyyy"); -> 2021 출력
...
// DB에 저장된 얼럿문구 가져오기
// 쿼리가 LIKE 문이기 때문에 조회 결과가 여러개가 될 수 있다.
// 조회결과 갯수에 따라 1개면 string, 2개 이상이면 map 형태로 return
function getNotiMsg(noti_no) {
	var resultMsg = "";
	
	var param = {'notiCtgry' : noti_no};
	$.ajax({
		url : '/common/getNotiMsg.do',
		type : 'post',
		dataType : 'json',
		data : param,
		async : false,
		success : function(response){
			if(response.resultStats.resultCode == 'ok'){
				if(response.resultStats.resultMsg == "") { 
					resultMsg = response.notiMap;
				} else {
					alert(response.resultStats.resultMsg);
				}
			} else {
