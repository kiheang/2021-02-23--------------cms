...
;(function($){
	$.fn.touchSwipe = function(opts){		
		...
		return this.each(function(){
			...
			this._banner_hgt = this.opts.banner_wrp.children().outerHeight();
			...
			this._banner_auto = this.opts.banner_auto;
			this._banner_time = this.opts.banner_time;
			if ( this._banner_time == null || isNaN(this._banner_time) ||this._banner_time < 0 ) {
				this._banner_time = 1500;
			}
			this._banner_fuc = null;
	...
	var touchSwipe = {
		setup : function(){
			...
			if ( this._banner_lbt && this._banner_rbt ) {
				this._banner_lbt.bind("click", function(e){
					_this.prev();
					e.preventDefault();
				});
				this._banner_rbt.bind("click", function(e){
					_this.next();
					e.preventDefault();
				});
				this._banner_lbt.css("z-index", 999);
				this._banner_rbt.css("z-index", 999);
			}
			
			if ( this._nv_btn ) {
				this._nv_btn.bind("click", function(){
					if (_this._browser == "msie" || _this._browser == "trident" ) {
			...
			if (this._browser == "msie" || _this._browser == "trident") {
				this._banner_idx = 1;
			}
...
//			});
		...
		auto : function(){
			var _this = this;
			if ( this._banner_auto ) {
				clearInterval(this._banner_fuc);
				this._banner_fuc = setInterval(function(){
					_this.next();
				}, this._banner_time);
				
				$(this).hover( 
					function(){
						clearInterval(_this._banner_fuc);
					}, function(){
						clearInterval(_this._banner_fuc);
						_this._banner_fuc = setInterval(function(){
							_this.next();
					}, _this._banner_time);
				});
			}
			...
			if((e.type == "touchmove" && e.originalEvent.touches.length <= 1) || (e.type == "mousemove" && this._mousedown) ||  (e.type == "drag" && e.originalEvent.pageX > 0) ) {
				this._deltaX = (e.originalEvent.pageX || e.originalEvent.touches[0].pageX) - this._start.pageX;
					...
					if (this._browser == "msie" || _this._browser == "trident" ) {
						...
						this._banner_wrp.css("-webkit-transform", "translate3d(" + (this._deltaX - this._banner_idx * this._banner_wid) + "px,0,0)");
					}
		...
		setTranslate3d : function(duration, posX) {
			this._banner_wrp.css({
				...
				"-ms-yransform" : "translate3d(" + posX + "px,0, 0)",
				"-o-transform" : "translate3d(" + posX + "px,0, 0)",
				"transform" : "translate3d(" + posX + "px,00, 0)",
//				"-webkit-transform" : "translate3d(" + posX + "px,0px,200px)",
//				"-moz-transform" : "translate3d(" + posX + "px,0px,20px)",
//				"-ms-yransform" : "translate3d(" + posX + "px,0px,20px)",
//				"-o-transform" : "translate3d(" + posX + "px,0px,20px)",
//				"transform" : "translate3d(" + posX + "px,0px,200px)"
